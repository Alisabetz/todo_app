...
{
    public class Profile : ToDo.DataLayer.Repositories.IRepository
    {
        ...
        public override bool Add(ModelsInterface model)
        {
            try
            {
                if (model is ToDo.DataLayer.Models.ProfileModel inputModel)
                {
                    GetTable();

                    DataRow newRow = table.NewRow();
                    newRow[1] = inputModel.ProfileName;
            ...
            try
            {
                ...
                if (table.Select($"{table.Columns[0].ColumnName} = {id}").Length > 0)
                {
                    if (model is ToDo.DataLayer.Models.ProfileModel inputModel)
                    {
                        ...
                        editedRow[1] = inputModel.ProfileName;
